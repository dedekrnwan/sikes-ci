...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_jurnal` AS SELECT 
	j.*
FROM t_jurnal j
JOIN v_pembayaran vp ON vp.tarif_nilai_id = j.tarif_nilai_id
WHERE j.active = 1 ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_siswa` AS SELECT 
	...
	ta.ta
FROM siswa s
JOIN siswa_status st ON s.siswa_id = st.siswa_id AND st.active = 1 
JOIN ta on ta.ta_id = st.ta_id and ta.active = 1
where s.active = 1 ;
